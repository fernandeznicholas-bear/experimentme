...
const proprietaryAssessments = [
  { name: 'MMPI-3', badge: 'Licensed', desc: 'Gold standard clinical personality', publisher: 'Pearson' },
  { name: 'NEO PI-R', badge: 'Licensed', desc: 'Comprehensive Big Five personality', publisher: 'PAR' },
  { name: '16PF', badge: 'Licensed', desc: 'Classic 16-factor personality', publisher: 'Pearson' },
  { name: 'MCMI-IV', badge: 'Licensed', desc: 'Personality disorders assessment', publisher: 'Pearson' },
  { name: 'PAI', badge: 'Licensed', desc: 'Clinical intake measure', publisher: 'PAR' },
  { name: 'BDI-II', badge: 'Licensed', desc: 'Famous depression inventory', publisher: 'Pearson' },
  { name: 'BAI', badge: 'Licensed', desc: 'Anxiety vs. depression', publisher: 'Pearson' },
  { name: 'Conners 4', badge: 'Licensed', desc: 'ADHD assessment', publisher: 'MHS' },
  { name: 'Strong Interest', badge: 'Licensed', desc: 'Career counseling staple', publisher: 'CPP' },
  { name: "Raven's Matrices", badge: 'Licensed', desc: 'Fluid intelligence', publisher: 'Pearson' },
  { name: 'SCID-5', badge: 'Clinician', desc: 'Diagnostic interview', publisher: 'APA' },
  { name: 'HAM-D', badge: 'Clinician', desc: 'Clinician-rated depression', publisher: 'Public' },
  { name: 'ADOS-2', badge: 'Clinician', desc: 'Autism observation', publisher: 'WPS' },
  { name: 'Bayley-4', badge: 'Clinician', desc: 'Infant development', publisher: 'Pearson' },
  { name: 'Rorschach', badge: 'Materials', desc: 'Inkblot projective test', publisher: 'Various' },
  { name: 'TAT', badge: 'Materials', desc: 'Picture card analysis', publisher: 'Various' },
  { name: 'WAIS-IV', badge: 'IQ', desc: 'Adult intelligence scale', publisher: 'Pearson' },
  { name: 'WISC-V', badge: 'IQ', desc: 'Child intelligence (6-16)', publisher: 'Pearson' },
  { name: 'Stanford-Binet', badge: 'IQ', desc: 'First modern IQ test', publisher: 'Riverside' },
  { name: 'WCST', badge: 'Neuropsych', desc: 'Executive function', publisher: 'PAR' },
  { name: 'Trail Making', badge: 'Neuropsych', desc: 'Processing speed', publisher: 'Public' },
  { name: 'Stroop Test', badge: 'Neuropsych', desc: 'Attention / inhibition', publisher: 'Various' },
  { name: 'CPT-3', badge: 'Neuropsych', desc: 'Sustained attention', publisher: 'MHS' },
]

const badgeStyles: Record<string, string> = {
  Licensed: 'bg-amber-100 text-amber-700',
  Clinician: 'bg-blue-100 text-blue-700',
  Materials: 'bg-purple-100 text-purple-700',
  IQ: 'bg-emerald-100 text-emerald-700',
  Neuropsych: 'bg-rose-100 text-rose-700',
}
...
export default function PrototypeClient() {
  ...
  const allItems = albums // always show all 15 + vault
  ...
  const handleScroll = useCallback(() => {
    ...
    setActiveIndex(Math.max(0, Math.min(idx, allItems.length)))
  }, [allItems.length])
  ...
  const activeAlbum = activeIndex < allItems.length ? allItems[activeIndex] : null
    ...
    <main className="min-h-screen bg-cream overflow-x-hidden">
      ...
      <div className="pt-28 pb-2 px-6 max-w-5xl mx-auto">
        <div className="flex items-start justify-between">
          <div>
            <h1 className="font-[family-name:var(--font-heading)] text-3xl md:text-4xl font-bold text-brown-deep">
              Explore Assessments
            </h1>
            <p className="font-[family-name:var(--font-body)] text-text-muted text-sm mt-1.5">
              15 free assessments + The Vault
            </p>
          </div>
      ...
      <div className="relative mt-6">
        ...
        >
                ...
                >
                  ...
                  <div className={`relative w-full rounded-2xl overflow-hidden ${album.bg} shadow-2xl`} style={{ height: 380, aspectRatio: '3/4' }}>
                    ...
                    <div className="absolute bottom-0 left-0 right-0 z-10 bg-gradient-to-t from-black/70 via-black/40 to-transparent pt-16 pb-5 px-5">
                      ...
                      {/* Begin Discovery button — only on active card */}
                      {dist === 0 && (
                        <Link
                          href={`/assess/${album.slug}`}
                          onClick={e => { if (preventClick.current) e.preventDefault() }}
                          className="inline-block mt-3 px-6 py-2.5 rounded-xl bg-white/20 backdrop-blur-md text-white text-sm font-semibold hover:bg-white/30 transition-colors border border-white/20"
                        >
                          Begin {researchMode ? 'Assessment' : 'Discovery'}
                        </Link>
                      )}
                    </div>
                  </div>
          ...
          <div
            ...
            style={{ width: CARD_W, zIndex: 20 - Math.abs(allItems.length - activeIndex) }}
            onClick={() => {
              if (preventClick.current) return
              if (activeIndex === allItems.length) {
                setShowVault(true)
              } else {
                goTo(allItems.length)
              }
            }}
          >
            <div
              className="transition-all duration-500 ease-out will-change-transform"
              style={{
                transform: `perspective(800px) scale(${Math.abs(allItems.length - activeIndex) === 0 ? 1 : Math.abs(allItems.length - activeIndex) === 1 ? 0.85 : 0.72}) rotateY(${allItems.length > activeIndex ? -40 : allItems.length < activeIndex ? 40 : 0}deg)`,
                opacity: Math.abs(allItems.length - activeIndex) === 0 ? 1 : Math.abs(allItems.length - activeIndex) === 1 ? 0.6 : 0.35,
              }}
            >
              <div className="relative w-full rounded-2xl overflow-hidden bg-gradient-to-br from-gray-600 via-gray-800 to-gray-950 shadow-2xl" style={{ height: 380 }}>
                ...
                <div className="absolute top-4 left-4 right-4 flex items-start justify-between z-10">
                  <span className="px-2.5 py-1 rounded-full bg-white/15 text-white text-[10px] font-bold uppercase tracking-wider backdrop-blur-md">
                    Reference Library
                  </span>
                  <span className="px-2 py-0.5 rounded-full bg-white/15 text-white text-[10px] font-medium backdrop-blur-md">
                    35 more
                  </span>
                </div>
                <div className="absolute inset-0 flex items-center justify-center z-10">
                ...
                <div className="absolute bottom-0 left-0 right-0 z-10 bg-gradient-to-t from-black/70 via-black/40 to-transparent pt-16 pb-5 px-5">
                  <h3 className="font-[family-name:var(--font-heading)] text-xl font-bold text-white leading-tight drop-shadow-lg">
                    {researchMode ? 'Professional & Licensed' : 'The Vault'}
                  </h3>
                  <p className="text-white/85 text-sm mt-1.5 leading-snug">
                    {researchMode ? '35 instruments requiring professional administration or licensing' : 'The ones behind the curtain. Tap to peek inside.'}
                  </p>
                  {Math.abs(allItems.length - activeIndex) === 0 && (
                    <button
                      onClick={e => { e.stopPropagation(); setShowVault(true) }}
                      className="inline-block mt-3 px-6 py-2.5 rounded-xl bg-white/20 backdrop-blur-md text-white text-sm font-semibold hover:bg-white/30 transition-colors border border-white/20 cursor-pointer"
                    >
                      Open The Vault
                    </button>
                  )}
      ...
      <div className="sm:hidden text-center py-6">
        <Link
          href="/prototype/dashboard"
          className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-brown-deep text-white text-sm font-medium"
        >
          Dashboard
        </Link>
      </div>
      ...
      {/* ── Vault Drawer ── */}
      {showVault && (
        <div className="fixed inset-0 z-50" onClick={() => setShowVault(false)}>
          <div className="absolute inset-0 bg-black/50 backdrop-blur-sm" />
          <div className="absolute inset-x-0 bottom-0 sm:inset-0 sm:flex sm:items-center sm:justify-center">
            <div
              className="relative bg-white rounded-t-2xl sm:rounded-2xl w-full sm:max-w-lg max-h-[85vh] overflow-hidden shadow-2xl"
              onClick={e => e.stopPropagation()}
            >
              <div className="sticky top-0 bg-white border-b border-brown-deep/5 px-6 py-5 flex items-center justify-between z-10">
                <div>
                  <h2 className="font-[family-name:var(--font-heading)] text-xl font-bold text-brown-deep">The Vault</h2>
                  <p className="text-text-muted text-xs mt-0.5">35 professional & licensed assessments</p>
                </div>
                <button onClick={() => setShowVault(false)} className="w-8 h-8 rounded-full bg-brown-deep/5 flex items-center justify-center text-text-muted hover:bg-brown-deep/10 transition-colors cursor-pointer text-lg">×</button>
              </div>
              <div className="px-6 py-4 overflow-y-auto max-h-[calc(85vh-80px)]">
                <p className="text-text-muted text-sm mb-5 leading-relaxed">
                  These require paid licenses, trained clinicians, or physical materials.
                  We teach you about them — what they measure, who uses them, and when you might encounter them.
                </p>
                <div className="space-y-2">
                  {proprietaryAssessments.map(a => (
                    <div key={a.name} className="flex items-center gap-3 p-3 rounded-xl bg-cream/50 hover:bg-cream transition-colors">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-0.5">
                          <span className="font-bold text-sm text-brown-deep">{a.name}</span>
                          <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider ${badgeStyles[a.badge] || 'bg-gray-100 text-gray-600'}`}>{a.badge}</span>
                        </div>
                        <p className="text-text-muted text-xs">{a.desc}</p>
                      </div>
                      <span className="text-text-muted text-[10px] font-mono shrink-0">{a.publisher}</span>
                    </div>
                  ))}
                </div>
              </div>
